...
namespace Engine {
	...
	//TEMP TODO
	VkPhysicalDevice VulkanRenderAPI::physicalDevice;
	VkDevice VulkanRenderAPI::device;									//logical device
	VkQueue VulkanRenderAPI::graphicsQueue;
	VkCommandPool VulkanRenderAPI::commandPool;

	RenderAPI * VulkanRenderAPI::Create()
	{
	...
	{
		EN_CORE_TRACE("vulkan render API");
		Init();
	}
	...
	void * VulkanRenderAPI::GetSceneFrameBufferTexture()
	{
		return nullptr;
	}

	void * VulkanRenderAPI::GetGameFrameBufferTexture()
	{
		return nullptr;
	}
	...
	void VulkanRenderAPI::CleanUp()
	{
		...
		for (VKVertexBuffer buffer:vertexBuffers)
		{
			buffer.CleanUp(device);
		}

		for (VKIndexBuffer buffer:indexBuffers)
		{
			buffer.CleanUp(device);
		}
		...
		glfwDestroyWindow((GLFWwindow*)Application::Get().GetWindow().GetNativeWindow());
		glfwTerminate();
	}
	...
	void VulkanRenderAPI::InitVulkan()
	{
		CreateInstance();
		SetupDebugCallback();
		CreateSurface();
		...
		CreateFrameBuffers();
		CreateTextureImage("../Engine/res/Textures/white.png");
		CreateTextureImageView();
		CreateTextureSampler();
		LoadModel("../engine/res/Models/killhouse.obj");
	...
	void VulkanRenderAPI::SetupDebugCallback()
	{
	...
	{
		if (glfwCreateWindowSurface(instance, (GLFWwindow*)Application::Get().GetWindow().GetNativeWindow(), nullptr, &surface) != VK_SUCCESS)
		{
	...
	void VulkanRenderAPI::CreateSwapChain()
	{
		...
		QueueFamilyIndices indices = FindQueueFamilies(physicalDevice);
		uint32_t queueFamilyIndices[] = { (uint32_t)
		indices.graphicsFamily, (uint32_t)indices.presentFamily };
	...
	void VulkanRenderAPI::CreateDescriptorSetLayout()
	{
	...
	{
		//shader Ch13
		auto vertShaderCode = ReadFile("../Engine/res/Shaders/vert.spv");
		auto fragShaderCode = ReadFile("../Engine/res/Shaders/frag.spv");
		...
		//fixed functions Ch14

		//vertex input
		auto bindingDescription = VulkanVertex::getBindingDescription();
		auto attributeDescription = VulkanVertex::GetAttributeDescriptions();
	...
	void VulkanRenderAPI::CreateTextureImage(const std::string& file_path)
	{
	...
	void VulkanRenderAPI::CreateTextureImageView()
	{
		textureImageView = CreateImageView(textureImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT);
	}

	void VulkanRenderAPI::CreateTextureSampler()
	{
		...
		samplerInfo.maxAnisotropy = 16; //perf
	...
	void VulkanRenderAPI::LoadModel(const std::string& file_path)
	{
		...
		if (!tinyobj::LoadObj(&attrib, &shapes, &materials, &err, &err, file_path.c_str()))
		{
		...
		VKVertexBuffer vb(device, vertices);
		VKIndexBuffer ib(device, indices);
	...
	void VulkanRenderAPI::CreateUniformBuffer(VkDeviceSize & bufferSize)
	{
		size_t size = uniformBuffers.size() + 1;

		EN_CORE_INFO("{0}", (int)size);
		...
		CreateBuffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
			...
			uniformBuffers[size-1], uniformBuffersMemory[size-1]);
	}

	void VulkanRenderAPI::CreateDescriptorPools()
	{
		CreateDescriptorPool(0);
		CreateDescriptorPool(1);
	}

	void VulkanRenderAPI::CreateDescriptorPool(uint32_t index)
	{
		size_t size = descriptorPools.size() + 1;

		descriptorPools.resize(size);
		...
		if (vkCreateDescriptorPool(device, &poolInfo, nullptr, &descriptorPools[index]) != VK_SUCCESS) {
			throw std::runtime_error("failed to create descriptor pool!");
		}
		
	}

	void VulkanRenderAPI::CreateDescriptorSets()
	{
		CreateDescriptorSet(0);
		CreateDescriptorSet(1);
	}

	void VulkanRenderAPI::CreateDescriptorSet(uint32_t index)
	{
		...
		size_t size = descriptorSets.size() + 1;

		descriptorSets.resize(size);
		VkResult result = vkAllocateDescriptorSets(device, &allocInfo, &descriptorSets[index]);
	...
	void VulkanRenderAPI::CreateCommandBuffers()
	{
		...
		for (size_t i = 0; i < commandBuffers.size(); i++)
		{
			...
			vertexBuffers[0].Bind(commandBuffers[i]);

			{
				vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSets[0], 0, nullptr);

				indexBuffers[0].Bind(commandBuffers[i]);
				vkCmdDrawIndexed(commandBuffers[i], indexBuffers[0].Count(), 1, 0, 0, 0);
			}
			{
	...
	void VulkanRenderAPI::UpdateUniformBuffer(uint32_t index)
	{
		...
		if (index == 0)
		{
			model = glm::translate(glm::mat4(1.0f), glm::vec3(-1.0f, 0, 0));
		...
		ubo.model = glm::rotate(model, time / 10 * glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
		ubo.model = glm::rotate(model, glm::radians(90.f), glm::vec3(1.0f, 0.0f, 0.0f));
	...
	void VulkanRenderAPI::CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer & buffer, VkDeviceMemory & bufferMemory)
	{
	...
	void VulkanRenderAPI::CopyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size)
	{
	...
	std::vector<const char*> VulkanRenderAPI::getRequiredExtension()
	{
		uint32_t glfwExtentionCount = 0;
		const char** glfwExtensions;
		glfwExtensions = glfwGetRequiredInstanceExtensions(&glfwExtentionCount);					//TODO
	...
	VkImageView VulkanRenderAPI::CreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags)
	{
	...
	std::vector<char> VulkanRenderAPI::ReadFile(const std::string & fileName)
	{
	...
	void VulkanRenderAPI::CreateImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage & image, VkDeviceMemory & imageMemory)
	{
	...
	void VulkanRenderAPI::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout)
	{
		VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
	...
	uint32_t VulkanRenderAPI::FindMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
	{
	...
	void VulkanRenderAPI::DestroyDebugUtilsMessengerEXT(VkInstance instance, VkDebugUtilsMessengerEXT callback, const VkAllocationCallbacks * pAllocator)
	{
	...
	std::array<VkVertexInputAttributeDescription, 4> VulkanVertex::GetAttributeDescriptions()
	{
		...
		attributeDescriptions[1].offset = offsetof(VulkanVertex, normals);
