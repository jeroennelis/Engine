...
namespace Engine{
	...
	VkDevice VulkanRenderer::device;									//logical device
	VkQueue VulkanRenderer::graphicsQueue;
	VkCommandPool VulkanRenderer::commandPool;

	VkCommandBuffer VulkanRenderer::m_CurrentCommandBuffer;
	...
	void* Engine::VulkanRenderer::GetSceneFrameBufferTexture()
	{
		return nullptr;
	}

	void* Engine::VulkanRenderer::GetGameFrameBufferTexture()
	{
		return nullptr;
	}
	...
	void VulkanRenderer::CreateTextureImage(const std::string& file_path)
	{
	...
	void VulkanRenderer::LoadModel(const std::string& file_path)
	{
		...
		VertexBuffer * vb = VertexBuffer::Create(vertices.data(), (int)vertices.size() * 11 * sizeof(float));
		IndexBuffer * ib = IndexBuffer::Create(indices.data(), (int)indices.size());
	...
	void VulkanRenderer::CreateUniformBuffer(VkDeviceSize& bufferSize)
	{
	...
	void VulkanRenderer::CreateDescriptorPool(uint32_t index)
	{
	...
	void VulkanRenderer::CreateDescriptorSet(uint32_t index)
	{
	...
	void VulkanRenderer::CreateCommandBuffers()
	{
		...
		for (size_t i = 0; i < commandBuffers.size(); i++)
		{
			...
			m_CurrentCommandBuffer = commandBuffers[i];
...

			{
				...
				indexBuffers[0]->Bind();
				vkCmdDrawIndexed(commandBuffers[i], indexBuffers[0]->GetCount(), 1, 0, 0, 0);
			}
			{
	...
	std::vector<const char*> VulkanRenderer::getRequiredExtension()
	{
